...
This project focuses on cleaning and preparing a layoffs dataset using SQL. The workflow includes removing duplicates, standardizing inconsistent values, handling nulls, and dropping unnecessary rows or columns. The goal is to transform raw data into a clean, analysis‑ready dataset using industry‑standard SQL techniques.

📁 Project Overview
This SQL project demonstrates a complete data‑cleaning pipeline using a real‑world dataset. Key tasks include:
...
The final output is a fully cleaned and structured dataset ready for analysis or visualization.
...
- Removed duplicates where row_num > 1
Key SQL Concepts:
- Window functions
- CTEs
...
- Converted date strings (MM/DD/YYYY) into SQL DATE format
Key SQL Concepts:
- String cleaning
...
3️⃣ Handling Null or Blank Values
Steps:
...
- Ensured consistent and complete data across the dataset
Key SQL Concepts:
- IS NULL
- Self‑joins
- Conditional updates

4️⃣ Removing Unnecessary Rows & Columns
Actions:
...
📊 Final Outcome
By the end of the project, the dataset was:
- Fully deduplicated
